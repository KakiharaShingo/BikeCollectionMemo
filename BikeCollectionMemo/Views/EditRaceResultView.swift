...
struct EditRaceResultView: View {
    ...
    private func loadCurrentData() {
        ...
        raceDate = raceResult.raceDate ?? Date()
        
        let currentTrack = raceResult.track ?? ""
        let currentCategory = raceResult.category ?? ""
        
        // 既存のコース/カテゴリーがプリセットにない場合は「その他」として扱う
        if viewModel.popularTracks.contains(currentTrack) {
            track = currentTrack
            customTrack = ""
        } else {
            track = "その他"
            customTrack = currentTrack
        }
        
        if viewModel.raceCategories.contains(currentCategory) {
        ...
        position = raceResult.position == 0 ? "" : String(raceResult.position)
        totalParticipants = raceResult.totalParticipants == 0 ? "" : String(raceResult.totalParticipants)
        bestLapTime = raceResult.bestLapTime ?? ""
        totalTime = raceResult.totalTime ?? ""
        weather = raceResult.weather ?? ""
        temperature = raceResult.temperature ?? ""
        notes = raceResult.notes ?? ""
        bikeName = raceResult.bikeName ?? ""
        
        // 既存の写真データをロード
        if let photos = raceResult.photos as? Set<RacePhoto> {
            let sortedPhotos = photos.sorted { $0.sortOrder < $1.sortOrder }
            imageDataArray = sortedPhotos.compactMap { $0.imageData }
        }
    }
    
    private func updateRaceResult() {
        let positionValue = Int32(position) ?? 0
        let totalParticipantsValue = Int32(totalParticipants) ?? 0
        
        viewModel.updateRaceResult(
            raceResult,
        ...
        // リストビューに即座に反映させるため、Core Dataの変更を通知
        DispatchQueue.main.async {
...
#Preview {
    ...
    sampleRaceResult.raceName = "関東エンデューロシリーズ"
    sampleRaceResult.raceDate = Date()
    sampleRaceResult.track = "オフロードヴィレッジ"
    sampleRaceResult.category = "クロスカントリー（XC）"
    sampleRaceResult.position = 3
    sampleRaceResult.totalParticipants = 25
    
    return EditRaceResultView(raceResult: sampleRaceResult)
