...
class MaintenanceRecordViewModel: ObservableObject {
    ...
    func updateMaintenanceRecord(
        ...
        saveContext(relatedBike: record.bike)
    }
    
    func deleteMaintenanceRecord(_ record: MaintenanceRecord) {
        let context = persistenceController.container.viewContext
        let relatedBike = record.bike
        context.delete(record)
        saveContext(relatedBike: relatedBike)
    }
    
    private func saveContext(relatedBike: Bike? = nil) {
        let context = persistenceController.container.viewContext
        
        // 関連するバイクオブジェクトを更新
        if let bike = relatedBike {
            bike.updatedAt = Date()
            context.refresh(bike, mergeChanges: true)
        }
...
class PartsMemoViewModel: ObservableObject {
    ...
    func updatePartsMemo(
        ...
        saveContext(relatedBike: memo.bike)
    }
    
    func deletePartsMemo(_ memo: PartsMemo) {
        let context = persistenceController.container.viewContext
        let relatedBike = memo.bike
        context.delete(memo)
        saveContext(relatedBike: relatedBike)
    }
